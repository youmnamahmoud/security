...
{0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15},{13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9},};
    int s3[4][16]= {{10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8},{13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1},{13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7},
{1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12},};
    int s4[4][16] = {{7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15},{13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9},{10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4},
...
{9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6},{4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13},};
    int s7[4][16]={{4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1},{13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6},{1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2},
...
string invper(string input){
int IP_1[64] = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25};      string output="",test;
    //test=hex_str_to_bin_str(input);
    for(int i=0;i<64;i++){
...
vector <string> keygeneration(string key){
    ...
    string pc1,upper,lower,upperafter,lowerafter,keyround,pc2;
    pc1=p1(key);
...
int main()
{
    string key,plaintext,ciphertext,initialinput,rightinput,leftinput,e,ss,p,x,xx,round,pc2,swaptoright,swaptoleft;
    int n;
    cin>>key;
    cin>>plaintext;
    cin>>n;
    ...
    if(n==0)
        ...
        vector <string> keys=keygeneration(key);
        for(int g=0;g<n;g++){
            initialinput=initialp(round);
            leftinput=initialinput.substr(0,32);
            rightinput=initialinput.substr(32,32);
            for(int j=0;j<16;j++){
                if(j!=0){
                    leftinput=round.substr(0,32);
                    rightinput=round.substr(32,32);
                }
                pc2=keys[15-j];
                e=exp(rightinput);
                x="000000000000000000000000000000000000000000000000";
                for(int i=0;i<48;i++){
                    if(e[i]=='0' && pc2[i]=='0') x[i]='0';
                    else if(e[i]=='1' && pc2[i]=='1') x[i]='0';
                    else if(e[i]=='1' && pc2[i]=='0') x[i]='1';
                    else if(e[i]=='0' && pc2[i]=='1') x[i]='1';
                }
                ss=s(x);
                p=per(ss);
                xx="00000000000000000000000000000000";
                for(int i=0;i<32;i++){
                    if(leftinput[i]=='0' && p[i]=='0') xx[i]='0';
                    else if(leftinput[i]=='1' && p[i]=='1') xx[i]='0';
                    else if(leftinput[i]=='1' && p[i]=='0') xx[i]='1';
                    else if(leftinput[i]=='0' && p[i]=='1') xx[i]='1';
                }
                round=rightinput+xx;
            }
            swaptoleft=round.substr(32,32);
            swaptoright=round.substr(0,32);
            round=swaptoleft+swaptoright;
            round=invper(round);

            ciphertext="";
            for(int i=0;i<round.length();i+=4){
                ciphertext+=bin_to_hex(round.substr(i,4));
            }
            round=ciphertext;
        }
